...
void canvas_copy_bound(struct canvas source, struct point s_from, struct point s_to, struct canvas* target, struct point t_from) {
    assert(source.map);
    assert(target);
    assert(target->map);
    ...
    int t_rows = target->height - t_from.y;
    int s_rows = s_to.y - s_from.y;
    int s_columns = s_to.x - s_from.x;
    int copy_bit_len = MIN(s_to.x - s_from.x, target->width - t_from.x);
    int r;
    for (r = 0; r < s_rows, r < t_rows; r++) {
